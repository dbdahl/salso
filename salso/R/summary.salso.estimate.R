...
#'
summary.salso.estimate <- function(
  object,
  alternative,
  orderingMethod = 1,
  ...
  ...
  if (missing(alternative)) {
    ...
    if (!isTRUE(identical(uniq, seq_len(length(uniq))))) {
      stop("The alternative clustering is not in canonical form.")
    }
  }
  loss <- attr(object, "info")$loss
  x <- if (!is.null(attr(object, "psm"))) {
    attr(object, "psm")
  } else {
    attr(object, "draws")
  }
  ...
  order <- if (orderingMethod == 1) {
    ...
    stop("Unsupported value for the 'orderingMethod' argument.")
  }
  # nClusters
