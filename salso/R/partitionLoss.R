...
#'
#' Given partitions \eqn{\pi*} and \eqn{\pi}, \code{\link{partitionLoss}}
...
#' \itemize{
#' \item \code{partitionLoss(p1, p2, "binder") = choose(length(p1), 2) * ( 1 - randi(p1, p2) )}
#' \item \code{partitionLoss(p1, p2, "pear") = 1 - arandi(p1, p2)}
#' \item \code{partitionLoss(p1, p2, "VI.lb") = vi.dist(p1, p2)}
#' }
#'
...
#'
#' @seealso \code{\link{partitionExpectedLoss}}
#'
#' @export
#' @examples
...
#'
#' partitionLoss(p1, p2, loss="binder")
#' partitionLoss(p1, p2, loss="pear")
#' partitionLoss(p1, p2, loss="VI.lb")
#'
#' all.equal(partitionLoss(p1, p2, "binder"), choose(length(p1), 2) * ( 1 - randi(p1, p2) ))
#' all.equal(partitionLoss(p1, p2, "pear"), 1 - arandi(p1, p2))
#' all.equal(partitionLoss(p1, p2, "VI.lb"), vi.dist(p1, p2))
#'
partitionLoss <- function(partition1, partition2, loss=c("binder", "pear", "VI.lb")[3]) {
  expectedLoss(partition1, mkAdjacencyMatrix(partition2), lossCode(loss))
}

mkAdjacencyMatrix <- function(partition) {
...
#' @export
#' @rdname partitionLoss
randi <- function(partition1, partition2) {
  1 - ( partitionLoss(partition1, partition2, "binder") / choose(length(partition1),2) )
}

#' @export
#' @rdname partitionLoss
arandi <- function(partition1, partition2) {
  1 - partitionLoss(partition1, partition2, "pear")
}

#' @export
#' @rdname partitionLoss
vi.dist <- function(partition1, partition2) {
  partitionLoss(partition1, partition2, "VI.lb")
}
