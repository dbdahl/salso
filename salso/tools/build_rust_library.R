...
target_option <- if (sysname == "Windows") {
  c("--target", "x86_64-pc-windows-gnu")
} else {
  NULL
}
...
for (run_counter in 1:2) {
  ...
  status <- system2("cargo",
                    c("build", offline_option, "--release", jobs_option, target_option))
  if (status != 0) {
...
if (cran_build) {
  message("Deleting temporary CARGO_HOME directory.")
  unlink(".cargo", recursive = TRUE, force = TRUE, expand = FALSE)
